...
inline void print_debug_stats(std::ostream& os, const debug_stats& stats) {
    ...
    os << "  Callers:\n";
    os << std::format("    Tasks executed: {}\n", stats.caller_stats.num_tasks_executed.load(std::memory_order_relaxed));
    os << std::format("    Tasks stolen:   {}\n", stats.caller_stats.num_tasks_stolen.load(std::memory_order_relaxed));
    for (size_t i = 0; i < stats.per_worker.size(); ++i) {
        const auto& ws = stats.per_worker[i];
        os << std::format("  Worker {:3}:\n", i);
        os << std::format("    Tasks executed: {}\n", ws->num_tasks_executed.load(std::memory_order_relaxed));
        os << std::format("    Tasks stolen:   {}\n", ws->num_tasks_stolen.load(std::memory_order_relaxed));
        os << std::format("    Total task time: {:.3f} ms\n", ws->total_task_time_ns.load(std::memory_order_relaxed) / 1'000'000.0);
    }
}
